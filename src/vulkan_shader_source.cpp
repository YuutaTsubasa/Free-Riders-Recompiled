...
std::optional<std::string> vulkan_shader_source(const std::string& hlsl) {
    ...
    text.insert(at + anchor.size(),
                "#define g_ScreenSpaceScale vk::RawBufferLoad<float2>(g_PushConstants.SharedConstants + 320)\n");

    remove_cbuffer(text, "VertexPalette");
    replace_indexing(text, "g_VertexPalette", [&](const std::string& index) {
        return "vk::RawBufferLoad<float4>(" + address + std::to_string(vulkan_palette_address_offset) + ") + uint64_t(" +
               index + ") * 16, 0x10)";
    });
    remove_cbuffer(text, "LoopConstants");
    replace_indexing(text, "g_LoopConstants", [&](const std::string& index) {
        return "vk::RawBufferLoad<int4>(" + address + std::to_string(vulkan_loop_address_offset) + ") + uint64_t(" +
               index + ") * 16, 0x10)";
    });
    return text;
}
